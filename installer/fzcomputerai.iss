...
;
;    /FORCEENGINE   Executa o instalador oficial do motor MESMO que a versao
;                   fixada ({#CuaDriverVersion}) ja esteja instalada. Sem este
;                   parametro o passo detecta a igualdade e nao baixa nada
;                   (o download do motor tem ~27 MB - repetir isso em cada
;                   auto-atualizacao da GUI seria desperdicio).
;
...
; ---------------------------------------------------------------------------
  ...
  ; VERSAO MINIMA do motor (nao e mais "a versao exata a instalar").
  ; 0.16 e a serie em que o endpoint HTTP passou a exigir token Bearer, o que
  ; a GUI usa para publicar o MCP com seguranca; abaixo disso o endpoint fica
  ; aberto. O criterio de EngineAtPinnedVersion e "igual ou mais novo", entao
  ; quem ja tem 0.17+ NAO e rebaixado. O submodulo cua/ deste repositorio
  ; ainda declara 0.8.3 e NAO reflete o binario oficial publicado.
  #define CuaDriverVersion "0.16.0"
#endif
...
brazilianportuguese.PrereqEngineLocal=instalado %1 (atualizacoes nao consultadas - use o botao abaixo)
...
brazilianportuguese.PrereqPlanInstall=Acao prevista: instalar o motor na versao fixada %1 (componente "motor" marcado).
brazilianportuguese.PrereqPlanNothing=Acao prevista: nada a fazer - a versao fixada %1 ja esta instalada (componente "motor" desmarcado; use /FORCEENGINE para reinstalar).
...
brazilianportuguese.PrereqRecheck=Verificar atualizacoes (internet)
...
english.PrereqEngineLocal=installed %1 (updates not checked - use the button below)
...
english.PrereqPlanInstall=Planned action: install the engine at the pinned version %1 (component "engine" selected).
english.PrereqPlanNothing=Planned action: nothing to do - pinned version %1 is already installed (component "engine" cleared; use /FORCEENGINE to reinstall).
...
english.PrereqRecheck=Check for updates (online)
...
brazilianportuguese.StatusEngineInstall=Instalando o motor cua-driver {#CuaDriverVersion} pelo instalador oficial do projeto Cua...
...
english.StatusEngineInstall=Installing the cua-driver {#CuaDriverVersion} engine through the official Cua project installer...
...
// online acontece somente quando o usuario clica em "Verificar atualizacoes".
// Falha ou saida vazia do check-update NUNCA e tratada como erro: apenas nao ha
// informacao de atualizacao para mostrar.
//
...
begin
  ...
  begin
    RC := RunAndCapture('cua-driver check-update --json', Json);
...
// Compara duas versoes "a.b.c" numericamente. Retorna <0, 0 ou >0.
// Existe porque comparar versao como TEXTO mente: '0.8.3' > '0.17.0' em ordem
// alfabetica, e era exatamente esse o defeito descrito abaixo.
...
// Verdadeiro quando a versao instalada atende ao minimo fixado neste setup.
//
// ANTES exigia igualdade EXATA com {#CuaDriverVersion}. Consequencia real: numa
// maquina com o motor 0.17.0 (mais novo que o pin 0.8.3) o instalador concluia
// "versao errada" e reinstalava a fixada, REBAIXANDO um motor que ja estava
// certo — e derrubando junto o suporte a token do endpoint HTTP, que so existe
// da serie 0.16 em diante. Agora a comparacao e NUMERICA e o criterio e
// "igual ou mais novo": ninguem e rebaixado por causa do pin.
function EngineAtPinnedVersion: Boolean;
begin
  Result := EngineFound and (CompareVersions(EngineVersion, '{#CuaDriverVersion}') >= 0);
end;
...
begin
  ...
  else if EngineAtPinnedVersion and (not ForceEngineRequested) then
    S := FmtMessage(CustomMessage('PrereqPlanNothing'), ['{#CuaDriverVersion}'])
  else
    S := FmtMessage(CustomMessage('PrereqPlanInstall'), ['{#CuaDriverVersion}']);
...
//
// Regra: se a versao FIXADA neste setup ja esta instalada, nao ha o que baixar
// -> componente desmarcado (e a pagina diz isso em "Acao prevista"). Em
// qualquer outro caso (ausente, versao diferente, ou /FORCEENGINE) ->
// componente marcado.
...
begin
  ...
  if EngineAtPinnedVersion and (not ForceEngineRequested) then
  begin
    WizardSelectComponents('!engine');
    Log('[prereq] componente "engine" DESMARCADO por default: versao fixada {#CuaDriverVersion} ja instalada.');
  end
  else
  begin
    WizardSelectComponents('engine');
...
begin
  ...
  Btn.Left    := 0;
  Btn.Top     := ScaleY(Y);
  Btn.Width   := ScaleX(120);
  Btn.Height  := ScaleY(23);
  Btn.Caption := CustomMessage('PrereqRecheck');
  Btn.OnClick := @RecheckClick;
  Y := Y + 32;
...
//   1) script embarcado {app}\cua-driver\install.ps1 -Release {#CuaDriverVersion}
//   2) fallback de rede: o MESMO script, do endpoint oficial cua.ai
//
...
begin
  ...
  Log('[engine] ===== PASSO DO MOTOR cua-driver: INICIO (silencioso=' +
      YesNo(WizardSilent) + ', versao fixada={#CuaDriverVersion}) =====');
  ...
  begin
    DoInstall := False;
    Log('[engine] instalador oficial NAO executado: a versao fixada ' +
        '{#CuaDriverVersion} ja esta instalada (download de ~27 MB ' +
        'desnecessario). Passe /FORCEENGINE para reinstalar de qualquer forma.');
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        // Caminho 1: script OFICIAL embarcado (auditavel em {app}\cua-driver).
        Params := '-NoProfile -ExecutionPolicy Bypass -File "' + ScriptPath +
                  '" -Release {#CuaDriverVersion}';
        if WizardSilent then
          Params := Params + ' -NoAutoStart';
        Log('[engine] caminho 1 (script oficial embarcado): ' + ScriptPath);
      end
      else if WizardSilent then
      begin
        // Caminho 2b: fallback de rede em modo silencioso. Baixa o MESMO
        // script do MESMO endpoint oficial para um arquivo temporario para
        // poder passar -NoAutoStart (o one-liner "irm | iex" nao aceita
        // parametros e disparaia UAC numa instalacao desassistida).
        Params := '-NoProfile -ExecutionPolicy Bypass -Command ' +
                  '"$env:CUA_DRIVER_RS_VERSION=''{#CuaDriverVersion}''; ' +
                  '$s = Join-Path $env:TEMP ''cua-driver-install.ps1''; ' +
                  'irm https://cua.ai/driver/install.ps1 -OutFile $s; ' +
                  '& $s -Release ''{#CuaDriverVersion}'' -NoAutoStart"';
        Log('[engine] caminho 2b (endpoint oficial cua.ai, baixado para arquivo, -NoAutoStart).');
      end
      else
      begin
        // Caminho 2a: fallback de rede interativo - one-liner OFICIAL,
        // exatamente como o projeto Cua documenta.
        Params := '-NoProfile -ExecutionPolicy Bypass -Command ' +
                  '"$env:CUA_DRIVER_RS_VERSION=''{#CuaDriverVersion}''; ' +
                  'irm https://cua.ai/driver/install.ps1 | iex"';
        Log('[engine] caminho 2a (endpoint oficial cua.ai, one-liner irm | iex).');
      end;
...
begin
  ...
  begin
    ...
    RunAndCapture('cua-driver autostart kick', Out1);
    Exit;
  end;
  ...
  RunAndCapture('cua-driver autostart enable', Out1);
  RunAndCapture('cua-driver autostart kick', Out1);
end;
