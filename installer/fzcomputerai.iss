...
brazilianportuguese.StatusTlsInit=Gerando o certificado auto-assinado do endpoint HTTPS (se ainda nao existir)...
...
english.StatusTlsInit=Generating the self-signed certificate for the HTTPS endpoint (if not present yet)...
...
; --- Certificado do endpoint HTTPS (auto-assinado) --------------------------
; "Na instalacao ou no primeiro run, o que vier primeiro": gera o par
; selfsigned.crt/.key em %APPDATA%\FzComputerAI\tls se ainda nao existir
; (idempotente; cert valido e mantido). Roda TAMBEM em modo silencioso
; (auto-upgrade). Resultado em %APPDATA%\FzComputerAI\tls\tls-init.log.
