...
- **Contrato de auth do motor 0.17.0 PROVADO no binário** (fim da disputa de documentação): sem `CUA_DRIVER_RS_MCP_HTTP_TOKEN` o `serve` **nem sobe** ("must be set to a host-generated bearer token"); com daemon no ar, POST sem `Authorization` = **401** `{"code":-32001}` e com `Bearer` = **200**. Token persistido em `HKCU\Environment`; a scheduled task o lê no logon.
