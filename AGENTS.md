...
- **Let's Encrypt: DNS-01 via API do Cloudflare (padrão) ou HTTP-01 na porta 80.** `instant-acme`, conta persistida em `letsencrypt-account.json`, renovação automática < 30 dias (checagem a cada 6 h em `poll_tls`). DNS-01 (v2.3.0) é o caminho para host em **rede interna**: TXT criado/removido pela API, espera de propagação via DoH, registro A pode apontar para IP privado. O token do Cloudflare vive **só** em `cloudflare-api-token.txt` (0600) na pasta dos certificados — nunca registro/log/argv. HTTP-01 exige DNS público + porta 80 alcançável (respondedor temporário em `0.0.0.0:80`). **Não** implemente DNS-01 "manual" que trave a UI esperando o usuário editar DNS. Use *staging* nos testes (o limite de rate da produção é real).
- **Porta ocupada = fallback, não ERRO.** `tls::start_tls_proxy_fallback` tenta a porta configurada e as 20 seguintes (pula a do motor), persiste a que ficou e loga o motivo. Medido: na máquina de referência a 8443 é reservada pelo sistema (`WSAEACCES`).
...
- **"Ligado" tem que significar "de pé" (NORMATIVO, v2.3.5).** `start_tls()` roda por evento (startup, *Aplicar*, ACME OK, toggle do OAuth). Evento que falha **não pode** deixar o recurso morto para sempre: `poll_tls` chama `tls_retry_if_down`, que a cada 30 s ressuscita o listener quando `tls_enabled` está ligado, `tls_proxy` está vazio, não há emissão ACME em curso e o certificado do modo atual **resolve de verdade**. Ao acrescentar qualquer estado "ligado" persistido, dê a ele o mesmo supervisor — a preferência do usuário é a fonte da verdade, não o resultado da primeira tentativa. Regressão real que originou a regra: emissão Let's Encrypt falhou, o listener parou, e continuou parado mesmo com `letsencrypt.crt` válido por 87 dias no disco; só voltava reabrindo o app.
...
### 1.3. OAuth 2.1 na frente do MCP — convenções (NORMATIVO, v2.3.0)
- **Onde vive**: `oauth.rs` (servidor: metadata, DCR, authorize, token, estado) + `HttpRewriter` em `tls.rs` (roteia os caminhos OAuth, reescreve `Authorization`, devolve 401 com `WWW-Authenticate` quando não há credencial). O motor **não** sabe que o OAuth existe: recebe sempre o seu próprio bearer.
- **PKCE S256 obrigatório**, `token_endpoint_auth_methods_supported=["none"]` (cliente público, sem client_secret), refresh token com **rotação** (o usado é invalidado). Não afrouxe isso para "facilitar" um conector: é o contrato da spec de autorização do MCP.
- **Senha de autorização**: gerada pelo app, mostrada UMA vez, guardada só como SHA-256 em `oauth-state.json` (0600). Nunca no registro, log, console ou argv. Senha errada responde a mesma página com atraso de 800 ms.
- **redirect_uri**: só `https://` ou `http://localhost`/`127.0.0.1` (ou esquema custom de app nativo); tem de bater exatamente com o registrado.
- **Passthrough**: `Authorization` que não é token OAuth passa intacto (pode ser o bearer do motor); `Transfer-Encoding: chunked` na requisição cai em passthrough sem reescrita (clientes MCP mandam JSON com Content-Length).
- **Cloudflare Access (OIDC) na frente da senha — opcional (v2.3.3)**: configuração só por `cloudflare-oidc.json` na pasta dos certificados (`client_id`, `client_secret` opcional, `discovery_url`, `callback_url`); o segredo **nunca** vai para registro, log, console ou argv. O app continua sendo **o** servidor OAuth do conector (broker): contra o Cloudflare ele usa `state` (sid) e PKCE **próprios**, e a pendência expira em 10 min. O `id_token` só pode ser lido sem validar assinatura porque vem **direto do token endpoint por TLS verificado** — nunca aceitar `id_token` vindo do navegador ou de query string. A lista de quem entra é a política do Access; a senha do app é o segundo fator. Sem o arquivo, ou com a descoberta falhando, o `/authorize` cai na senha — **nunca** em "sem autenticação". Teste: `oauth::jwt_claim_reads_email`; ao mexer no callback, medir com `curl` o 302 do `/authorize` e o 400 "Sessão expirada" do `/oauth/cf/callback` sem pendência.
- **Testes**: `cargo test --bin fzcomputerai oauth::` e `tls::oauth_through_proxy_end_to_end`. Mantenha os dois verdes ao mexer em `oauth.rs`/`HttpRewriter`.
