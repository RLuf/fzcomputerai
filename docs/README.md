...
| [https.md](https.md) | Ligar o **HTTPS do endpoint** (terminação TLS no app): auto-assinado gerado no setup/primeiro run, Let's Encrypt (HTTP-01, renovação automática), certificado próprio, OAuth 2.1 para conectores, login pelo **Cloudflare Access (OIDC)**, como o cliente confia, limites e solução de problemas. |
