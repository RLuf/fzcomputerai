...
| `/DCuaDriverVersion=` (default `0.16.0`) | a versão **mínima** do motor aceita na instalação (comparação numérica, "igual ou mais novo" — quem já tem 0.17+ não é rebaixado) |

Outros comportamentos deliberados do instalador: `PrivilegesRequired=lowest` (instalação por usuário, em `{autopf}`); o **componente `engine`** (que substituiu a antiga task `cuadriver`) vem **marcado** por padrão, porque sem o motor nenhum botão funciona; ele roda em `InstallEngineStep` — **inclusive em instalação silenciosa** (`/VERYSILENT`), sendo pulado apenas com `/SKIPENGINE`; e `{app}\tunnel` (binários baixados sob demanda, token-file, policy do ngrok) é removido na desinstalação.
