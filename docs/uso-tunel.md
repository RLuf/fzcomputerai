...
A GUI **não** classifica nenhuma dessas opções como "segura" por si. Segurança aqui vem do que você põe na frente do MCP: senha na URL, autenticação de borda, ou o token exigido pelo próprio motor (medido em 2026-08-03 no `cua-driver` 0.17.0: sem `Authorization: Bearer <token>`, a resposta é 401). Esse token é gerado e gravado pela própria GUI desde a **v2.1.1**, e nunca é exibido em log ou na tela — para passá-lo ao cliente MCP remoto, leia com `reg query HKCU\Environment /v CUA_DRIVER_RS_MCP_HTTP_TOKEN`.
...
| bloco no `shutdown_cleanup` | o resto do fechamento normal: porteiro de senha, encaminhamento LAN e `cua-driver stop`. Desde a **v2.1.1** isso é Rust nativo executado neste processo — a versão anterior disparava um `powershell -WindowStyle Hidden` destacado (esperar a GUI morrer + matar processos + escrever no registro + `-Verb RunAs`), linha de comando que o Microsoft Defender flagrou nesta máquina (detecção 2147941383, em 2026-08-03) |
...
Um processo só é morto com **identidade de 3 fatores**: imagem (`cloudflared.exe`/`ngrok.exe`/`ssh.exe`) + `CreationDate` do processo + o marcador `run_id` presente na linha de comando (ele aparece no caminho do arquivo de log passado ao CLI). Isso elimina PID reciclado e, principalmente, protege o `cloudflared`/`ngrok`/`ssh` legítimo do usuário. **`taskkill /IM` é proibido no projeto** exatamente por isso — e foi por essa mesma regra que o `taskkill /F /IM cua-driver.exe` que sobrava no app e no instalador caiu na v2.1.1: ele matava **todo** processo com esse nome, inclusive um motor que você estivesse usando para outra coisa. O encerramento do motor é `cua-driver stop` e, se sobrar processo, kill por PID com o caminho do executável conferido.
...
| URL pública responde **502** | o porteiro/borda está de pé mas o MCP local não responde: motor parado, ou a porta mudou depois que o túnel subiu. Sem `CUA_DRIVER_RS_MCP_HTTP_TOKEN` no ambiente do processo, o `cua-driver serve` sai com código 1 e a porta fica muda (medido em 2026-08-03). Desde a **v2.1.1** a GUI gera o token e injeta porta e token no `serve` que ela mesma lança como processo filho, então esse caso ficou restrito ao daemon subido pela Tarefa Agendada `cua-driver-serve`, que herda o ambiente do **logon** | verifique o badge na aba MCP & Rede, religue o motor pela GUI (o log do motor aparece no console com o prefixo `[motor]`) e reinicie o túnel |
